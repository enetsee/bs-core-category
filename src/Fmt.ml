...
let brk spaces indent = Format.pp_print_break spaces indent 

let sp = Format.pp_print_space

let comma ppf () = pf ppf ",@ "

let const (pp_v: 'a t) v ppf () = pf ppf "%a" pp_v v
...
let stack ?sep:_ pp_elt = iter Stack.iter pp_elt
...
let bbox ?(indent = 0) pp ppf =
  Format.pp_open_box ppf indent; pf ppf "%a@]" pp
