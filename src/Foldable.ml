...
module CompleteCustom3_foldLeft(X:Custom3) : Complete3 with type ('a,'b,'c) t := ('a,'b,'c) X.t = struct 
  ...
  let foldMap_ (type a) (module M : Monoid.S0 with type t = a)
      ?init:(empty = M.mempty) x  ~f =
...
module CompleteCustom3_foldRight(X:Custom3_foldRight) : Complete3 with type ('a,'b,'c) t := ('a,'b,'c) X.t = struct 
...
module MakeComplete3(X:Complete3) : S3 with type ('a,'b,'c) t := ('a,'b,'c) X.t = struct 
  include X
...
module MakeCustom2_foldRight(X:Custom2_foldRight) : S2 with type ('a,'b) t := ('a,'b) X.t = MakeCustom3_foldRight(struct
  type ('a,'b,_) t = ('a,'b) X.t
  include (X: Custom2_foldRight with type ('a,'b) t := ('a,'b) X.t)
end)
  
module MakeCustom1_foldRight(X:Custom1_foldRight) : S1 with type 'a t := 'a X.t = MakeCustom2_foldRight(struct
  type ('a,_) t = 'a X.t
...
module Make3_foldRight(X:Minimal3_foldRight) : S3 with type ('a,'b,'c) t := ('a,'b,'c) X.t = MakeCustom3_foldRight(struct 
  include X
  let foldLeft = `Derived
  let foldMap = `Derived
end)

module Make2_foldRight(X:Minimal2_foldRight) : S2 with type ('a,'b) t := ('a,'b) X.t = Make3_foldRight(struct
  type ('a,'b,_) t = ('a,'b) X.t
  include (X: Minimal2_foldRight with type ('a,'b) t := ('a,'b) X.t)
end)
  
module Make1_foldRight(X:Minimal1_foldRight) : S1 with type 'a t := 'a X.t = Make2_foldRight(struct
  type ('a,_) t = 'a X.t
